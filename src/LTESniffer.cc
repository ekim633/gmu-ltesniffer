...
#undef NEW_MESSAGE
            ...
            if (display_rbs[jammed_prb_index].allocated && jammed_prb_index >= 0) {
                float jam_freq = display_rbs[jammed_prb_index].frequency;
                jammer_usrp->set_tx_freq(jam_freq);
                jammer_usrp->set_tx_rate(15.36e6);
                jammer_usrp->set_tx_gain(40.0f);

                const size_t spp = jammer_tx_stream->get_max_num_samps();
                std::vector<std::complex<int8_t>> jam_samples(spp, {100, -100});

                uhd::time_spec_t now = jammer_usrp->get_time_now();
                uhd::time_spec_t tx_time = now + uhd::time_spec_t(0.1);
                ...
                for (int i = 0; i < 5; ++i) {
                    jammer_tx_stream->send(&jam_samples.front(), spp, md);
