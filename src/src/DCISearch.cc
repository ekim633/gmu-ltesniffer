...
extern uint32_t global_rb_allocation_indices[MAX_NOF_PRBS];
extern uint32_t global_rb_allocation_indices_count;
extern bool global_rb_allocation_indices_changed;
extern std::mutex global_rb_allocation_indices_mutex;
...
int DCISearch::inspect_dci_location_recursively(srsran_dci_msg_t *dci_msg,
    ...
    uint hist_max_format_value = 0;
    uint nof_cand_above_threshold = 0;
    ...
    srsran_cell_t *cell = &falcon_ue_dl.q->cell;
            int result = srsran_pdcch_decode_msg_limit_avg_llr_power(&falcon_ue_dl.q->pdcch, &cand[format_idx].dci_msg,
                                                                     cce_map[ncce].location[L],
                                                                     meta_formats[format_idx]->format, cfi,
                                                                     &cand[format_idx].rnti, 0, cell, sf,
                                                                     &ue_dl_cfg->cfg.dci);
      ...
      printf("Cand. %d (sfn %d.%d, ncce %d, L %d, f_idx %d)\n", &cand[format_idx].rnti, sfn, sf_idx, ncce, L, format_idx);
#endif
            if (rntiManager.getActivationReason(cand[format_idx].rnti) == RM_ACT_RAR && cand[format_idx].dci_msg.format
                == 0) {
                ...
                for (auto dci_member: temp_dci0) {
                    if (dci_member.format == cand[format_idx].dci_msg.format && dci_member.rnti == cand[format_idx].rnti
                        && dci_member.ncce == ncce) {
            ...
            if ((meta_formats[format_idx]->format == SRSRAN_DCI_FORMAT1C) && (cand[format_idx].rnti > SRSRAN_RARNTI_END)
                && (cand[format_idx].rnti < SRSRAN_PRNTI)) {
            ...
            if (enableShortcutDiscovery &&
                ...
                INFO("RNTI matches to parent DCI cand. %d (format_idx %d), this: L%d ncce %d\n", cand[format_idx].rnti,
                     meta_formats[format_idx]->global_index, L, ncce);
                falcon_free_candidates(cand);
                return -(static_cast<int>(format_idx) + 1);
            }

            // Filter 1b: Illegal DCI positions
            cand[format_idx].search_space_match_result = srsran_pdcch_validate_location(
                srsran_pdcch_nof_cce(&falcon_ue_dl.q->pdcch, cfi), ncce, L, sf_idx, cand[format_idx].rnti);
      ...
      uint occurence = rnti_histogram_get_occurence(&q->rnti_histogram[format_idx], cand[format_idx].rnti);
      // Online max-search
      if(occurence > q->rnti_histogram[format_idx].threshold) {
        ...
        if(occurence > hist_max_format_value) {
          ...
          hist_max_format_idx = (int)format_idx;
            ...
            if (rntiManager.validateAndRefresh(cand[format_idx].rnti, meta_formats[format_idx]->global_index)) {
                ...
                hist_max_format_value = rntiManager.getFrequency(cand[format_idx].rnti,
                                                                 meta_formats[format_idx]->global_index);
            }
#endif
        } //end for (format) loop
        ...
        if (nof_cand_above_threshold > 1) {
            ...
            for (uint32_t format_idx = 0; format_idx < nof_formats; format_idx++) {
                if (cand[format_idx].rnti != FALCON_ILLEGAL_RNTI) {
                    ...
                    INFO("\t DCI cand. %d (format_idx %d) L%d ncce %d freq %d\n", cand[format_idx].rnti, format_idx, L,
                         ncce, hist);
                    if (hist > hist_max) {
                        hist_max = hist;
                        hist_max_format_idx = static_cast<int>(format_idx);
                        //hist_max_format_value = rnti_manager_getFrequency(q.rnti_manager, cand[format_idx].rnti, meta_formats[format_idx]->global_index);
                        hist_max_format_value = rntiManager.getFrequency(
                            cand[format_idx].rnti, meta_formats[format_idx]->global_index);
                    }
                }
            }
            if (hist_max_format_idx == -1) {
                INFO(
                    "\t Multiple valid DCI candidates for same CCE location, but none appear in histogram. Skipping all\n")
                ;
                nof_cand_above_threshold = 0;
            } else {
                INFO("\t Selected DCI cand. %d (format_idx %d)\n", cand[hist_max_format_idx].rnti,
                     meta_formats[hist_max_format_idx]->global_index);
            }
        }
        ...
        if (nof_cand_above_threshold > 0 &&
            ...
            if (L > 0 && max_depth > 0) {
                disambiguation_count = inspect_dci_location_recursively(dci_msg, cfi, cce_map, location_list,
                                                                        ncce + (1 << (L - 1)), L - 1, max_depth - 1,
                                                                        meta_formats, nof_formats, 0, nullptr);
            }
        ...
        // if no candidates were found, descend covered CCEs recursively
            if (nof_cand_above_threshold == 0) {
                ...
                if (L > 0 && max_depth > 0) {
                    ...
                    recursion_result += inspect_dci_location_recursively(
                        dci_msg, cfi, cce_map, location_list, ncce, L - 1, max_depth - 1, meta_formats, nof_formats,
                        enable_discovery, cand);
                    if (recursion_result < 0) {
                        //shortcut taken, activate RNTI
                        INFO("Shortcut detected: RNTI: %d (format_idx %d)!\n", cand[-recursion_result - 1].rnti,
                             -recursion_result - 1);
                        ...
                        hist_max_format_value = rntiManager.getFrequency(
                            cand[hist_max_format_idx].rnti,
                            meta_formats[static_cast<uint32_t>(hist_max_format_idx)]->global_index);
                        ...
                        if (cand[hist_max_format_idx].search_space_match_result ==
                            SEARCH_SPACE_MATCH_RESULT_AMBIGUOUS) {
                            //descend only right half; pass nullptr as parent_rnti_cands; disable further shortcuts
                            disambiguation_count = inspect_dci_location_recursively(
                                dci_msg, cfi, cce_map, location_list, ncce + (1 << (L - 1)), L - 1,
                                MIN(max_depth, DCI_DISAMBIGUATION_DEPTH) - 1, meta_formats, nof_formats, 0, nullptr);
                            if (disambiguation_count > 0) {
                                INFO("Disambiguation discovered %d additional DCI\n", disambiguation_count);
                            }
                        }
                        ...
                        rntiManager.activateAndRefresh(cand[hist_max_format_idx].rnti,
                                                       meta_formats[static_cast<uint32_t>(hist_max_format_idx)]->
                                                       global_index, RM_ACT_SHORTCUT);
                    } else {
                        //descend right half; pass NULL as parent_rnti_cands
                        recursion_result += inspect_dci_location_recursively(
                            dci_msg, cfi, cce_map, location_list, ncce + (1 << (L - 1)), L - 1, max_depth - 1,
                            meta_formats, nof_formats, enable_discovery, nullptr);
                    }
                }
        ...
        if (nof_cand_above_threshold > 0) {
            ...
            rntiManager.addCandidate(cand[hist_max_format_idx].rnti,
                                     meta_formats[(uint32_t) hist_max_format_idx]->global_index);
#endif
            ...
            uint32_t target_rnti = global_target_rnti;
            global_target_rnti_mutex.unlock();

            if (cand[hist_max_format_idx].rnti == target_rnti) { // TODO: track changing RNTI, specify target from command line
                dci_candidate_t *target = &cand[hist_max_format_idx];

                std::lock_guard<std::mutex> guard{global_rb_allocation_indices_mutex};
                global_rb_allocation_indices_changed = true;
                global_rb_allocation_indices_count = 0;

                //  see https://www.sharetechnote.com/html/Handbook_LTE_RAType.html
                bool is_ul = false;
                const char *type_str = "<Type 0, 1, 2?>";

                // TODO: use builtin version of below srsran_ra_dl_grant_to_grant_prb_allocation
                srsran_cell_t *cell = &falcon_ue_dl.q->cell;
                if (target->dci_msg.format == SRSRAN_DCI_FORMAT0) {
#if 0
                    is_ul = true;

                    // Uplink
                    srsran_dci_ul_t dci;
                     // TODO: can cell and ue_dl_cfg->cfg.dci be used here? they sound like they are for downlink...?
                    // TODO: related to the above concern - what is the difference between DCIs and UCIs then?
                    if (srsran_dci_msg_unpack_pusch(cell, sf, &ue_dl_cfg->cfg.dci, &target->dci_msg, &dci) == SRSRAN_SUCCESS) {
                        bool valid = true;
                        if (dci.ra_type_present) {
                            if (dci.ra_type == SRSRAN_RA_ALLOC_TYPE2) {
                                type_str = "Type 2";
                            } else {
                                valid = false;
                                ERROR("UL DCI has RA type that isn't type 2?! (it was type %d)", int(dci.ra_type - SRSRAN_RA_ALLOC_TYPE0));
                            }
                        } else {
                            type_str = "Unk Type";
                        }
                        ...
                        if (valid) {
                            if (dci.type2_alloc.mode == srsran_ra_type2_t::SRSRAN_RA_TYPE2_LOC) {
                                uint resource_block_start = (uint)floor((float)dci.type2_alloc.riv / float(cell->nof_prb - 1));
                                global_rb_allocation_indices_count = (dci.type2_alloc.riv % (cell->nof_prb - 1)) + 1;
                                if (global_rb_allocation_indices_count >= ARRAY_SIZE(global_rb_allocation_indices)) {
                                    global_rb_allocation_indices_count = ARRAY_SIZE(global_rb_allocation_indices) - 1;
                                    ERROR("Ran out of space to store RB indices");
                                }
                                for (uint i = 0; i < global_rb_allocation_indices_count; ++i) {
                                    global_rb_allocation_indices[i] = resource_block_start + i;
                                }
                            } else {
                                ERROR("Decoding of distributed allocations for SRSRAN_RA_ALLOC_TYPE2 is not yet implemented\n");
                            }
                        }
                    }
#endif
                    ...
                    // Downlink
                    srsran_dci_dl_t dci;
                    if (srsran_dci_msg_unpack_pdsch(cell, sf, &ue_dl_cfg->cfg.dci, &target->dci_msg, &dci) == SRSRAN_SUCCESS) {
                        // float system_bandwidth = 10.0f; // TODO: get from somewhere instead of hardcoding
                        // uint rbg_size = 0;
                        // if (float_equals(system_bandwidth, 1.4f)) {
                        //     rbg_size = 1;
                        // } else if (float_equals(system_bandwidth, 3.0f)
                        //     || float_equals(system_bandwidth, 5.0f)) {
                        // rbg_size = 2;
                        // } else if (float_equals(system_bandwidth, 10.0f)) {
                        //     rbg_size = 3;
                        // } else if (float_equals(system_bandwidth, 15.0f)
                        //     || float_equals(system_bandwidth, 20.0f)) {
                        //     rbg_size = 4;
                        // } else {
                        //     ERROR("invalid system bandwidth\n");
                        // }

                        uint rbg_size = 0;
                        if (cell->nof_prb <= 10) {
                            rbg_size = 1;
                        } else if (cell->nof_prb <= 26) {
                            rbg_size = 2;
                        } else if (cell->nof_prb <= 63) {
                            rbg_size = 3;
                        } else if (cell->nof_prb <= 110) {
                            rbg_size = 4;
                        } else {
                            assert(!"Invalid number of prbs");
                        }

                        switch (dci.alloc_type) {
                        case SRSRAN_RA_ALLOC_TYPE0: {
                            type_str = "Type 0";
                            uint possible_prb_count = ceil((float)cell->nof_prb / (float)rbg_size);
                            assert(possible_prb_count <= sizeof(dci.type0_alloc.rbg_bitmask) * CHAR_BIT);

                            if (rbg_size > 0) {
                                uint prb_index = 0;
                                for (int bit_index = possible_prb_count - 1; bit_index >= 0; bit_index--) {
                                    bool ended_early = false;
                                    if (dci.type0_alloc.rbg_bitmask & (1 << bit_index)) {
                                        for (uint current_prb_index = 0; current_prb_index < rbg_size; ++current_prb_index) {
                                            uint index = prb_index + current_prb_index;
                                            if (index >= cell->nof_prb) {
                                                ended_early = true;
                                                break;
                                            }
                                            global_rb_allocation_indices[global_rb_allocation_indices_count++] = index;
                                        }
                                    }
                                    if (ended_early) assert(bit_index == 0); // If we ended early, this should be the last RBG
                                    prb_index += rbg_size;
                                }
                            }
                        } break;
                        case SRSRAN_RA_ALLOC_TYPE1: {
                            type_str = "Type 1";
                            uint rb_in_subset_count = 0;
                            uint test = (int)std::floor(float(cell->nof_prb - 1) / (float)rbg_size) % rbg_size;
                            if (dci.type1_alloc.rbg_subset < test) {
                                rb_in_subset_count = (int)std::floor(float(cell->nof_prb - 1) / (float)(rbg_size * rbg_size)) * rbg_size + rbg_size;
                            } else if (dci.type1_alloc.rbg_subset > test) {
                                rb_in_subset_count = (int)std::floor(float(cell->nof_prb - 1) / (float)(rbg_size * rbg_size)) * rbg_size;
                            } else {
                                rb_in_subset_count = (int)std::floor(float(cell->nof_prb - 1) / (float)(rbg_size * rbg_size)) * rbg_size + ((cell->nof_prb - 1) % rbg_size) + 1;
                            }
                            uint possible_prb_count = (int)std::ceil(cell->nof_prb / rbg_size) - (int)std::ceil(std::log2(rbg_size)) - 1;

                            uint shift_amount = rb_in_subset_count - possible_prb_count;

                            assert(dci.type1_alloc.rbg_subset < rbg_size);
                            int num_bits = sizeof(dci.type1_alloc.vrb_bitmask) * CHAR_BIT;
                            int stride = rbg_size * rbg_size; // rbg_size * number of subsets
                            uint rbg_index = dci.type1_alloc.shift ? shift_amount / rbg_size : 0;
                            uint prb_index = dci.type1_alloc.shift ? shift_amount % rbg_size : 0;

                            for (int bit_index = num_bits - 1; bit_index >= 0; bit_index--) {
                                if (dci.type1_alloc.vrb_bitmask & (1 << bit_index)) {
                                    global_rb_allocation_indices[global_rb_allocation_indices_count++] = rbg_index * stride + dci.type1_alloc.rbg_subset * rbg_size + prb_index;
                                    if (global_rb_allocation_indices_count >= ARRAY_SIZE(global_rb_allocation_indices)) {
                                        global_rb_allocation_indices_count = ARRAY_SIZE(global_rb_allocation_indices) - 1;
                                        bit_index = -1;
                                        ERROR("Ran out of space to store RB indices");
                        ...
                        case SRSRAN_RA_ALLOC_TYPE2:
                            ...
                            break;
                        default:
                            ERROR("dci_msg_unpack_pdsch returned invalid alloc type\n");
                        }
                    }
                }


                printf("Decoded %s %s DCI from RNTI %d\n", is_ul ? "UL" : "DL", type_str, target->rnti);
                if (global_rb_allocation_indices_count == 0) {
                    printf("No resource blocks reserved?!\n");
                } else {
                    printf("Alloc (len=%u): [ ", global_rb_allocation_indices_count);
                    for (uint i = 0; i < global_rb_allocation_indices_count; i++) {
                        assert(global_rb_allocation_indices[i] < cell->nof_prb);
                        uint rb = global_rb_allocation_indices[i];
                        printf("%u ", rb);
                    }
                    printf("]\n");
                }
            ...
            if (cand[hist_max_format_idx].rnti != 0) {
                if (cand[hist_max_format_idx].dci_msg.format == 0) {
                    for (auto dci0_mem: temp_dci0) {
                        ...
                        if (dci0_mem.format == cand[hist_max_format_idx].dci_msg.format && dci0_mem.rnti == cand[
                                hist_max_format_idx].rnti && dci0_mem.ncce == ncce) {
                            add_dci = false;
                        }
                    }
                    if (add_dci) {
                        ...
                        dciCollection.addCandidate(cand[hist_max_format_idx], srsran_dci_location_t{L_disamb, ncce},
                                                   hist_max_format_value,
                ...
                for (auto dci0_mem: temp_dci0) {
                    ...
                    uint temp_hist_max_format_value = rntiManager.getFrequency(dci0_mem.rnti, dci0_mem.format);
                    // Prevent adding multiple times
                    if (dci0_mem.added == false) {
                        dciCollection.addCandidate(dci0_mem.cand, srsran_dci_location_t{dci0_mem.L, dci0_mem.ncce},
                                                   temp_hist_max_format_value,
                                                   sf, &ue_dl_cfg->cfg.dci);
                        dci0_mem.added = true;
                    }
                }
...
int DCISearch::recursive_blind_dci_search(srsran_dci_msg_t *dci_msg,
    ...
    nof_locations = srsran_pdcch_ue_locations_all_map(&falcon_ue_dl.q->pdcch, locations, MAX_CANDIDATES_BLIND, cce_map,
                                                      MAX_NUM_OF_CCE, sf_idx, cfi);
    ...
    for (uint location_idx = 0; location_idx < nof_locations; location_idx++) {
...
DCISearch::DCISearch(falcon_ue_dl_t &falcon_ue_dl,
                     ...
                     srsran_ue_dl_cfg_t *ue_dl_cfg) : falcon_ue_dl(falcon_ue_dl),
